...
class UrbanPlanningAgent(AgentPPO):
    ...
    def sample_worker(self, pid, queue, num_samples, mean_action, policy_state_dict):
        ...
        self.thread_loggers[pid].info(f'worker {pid} deal with {num_samples} samples. threads:{torch_threads}') 
        ...
        while logger.num_steps < num_samples:
            ...
            for t in range(10000):
                try:
                    ...
                    #self.thread_loggers[pid].info(f'worker {pid} with mean_action: {use_mean_action}')
                    action = self.policy_net.select_action(state_var, use_mean_action).numpy().squeeze(0)
                    #self.thread_loggers[pid].info(f'worker {pid} with action: {action}')
    ...
    def log_optimize_policy(self, iteration, info):
        ...
        tb_logger.add_scalar('train/train_R_shaping_eps_avg', log.avg_episode_shaping_reward, iteration)
        ...
        tb_logger.add_scalar('eval/eval_R_shaping_eps_avg', log_eval.avg_episode_shaping_reward, iteration) 
